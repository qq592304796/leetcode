...
//
//
// Related Topics 数组 哈希表 二分查找 动态规划 排序
// 👍 176 👎 0
...
class Solution {
    public long maxTaxiEarnings(int n, int[][] rides) {
        // 动态规划
        long[] profit = new long[rides.length + 1];
        // 按照结束位置、开始位置、小费进行排序
        Arrays.sort(rides, Comparator.comparingInt((int[] o) -> o[1]));
        for (int i = 0; i < rides.length; i++) {
            // 用二分法查找，上一个乘客下车的点
            int j = binarySearch(rides, i, rides[i][0]);
            // 跟前一个地点的收益进行对比，取最大收益的值
            profit[i + 1] = Math.max(profit[i], profit[j] + rides[i][1] - rides[i][0] + rides[i][2]);
        }
        return profit[rides.length];
    }
