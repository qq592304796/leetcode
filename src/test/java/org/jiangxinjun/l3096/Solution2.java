package org.jiangxinjun.l3096;//给你一个长度为 n 的二进制数组 possible 。
//
...
class Solution2 {
    public int minimumLevels(int[] possible) {
        // 先计算一个人玩所有关卡的得分
        int a = getScores(possible[0]);
        int b = Arrays.stream(possible).reduce(0, (left, right) -> left + getScores(right));
        b -= a;
        for (int i = 1; i < possible.length; i++) {
            if (a > b) {
                return i;
            }
            a += getScores(possible[i]);
            b -= getScores(possible[i]);
        }
        return -1;
    }

    private static int getScores(int v) {
        return v == 0 ? -1 : 1;
    }
